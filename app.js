...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Date Select Dropdowns Population
    ...
    const popDrops = () => {
        ...
        accs.forEach(a => { 
            bS?.insertAdjacentHTML('beforeend', `<option value="${a.id}">${a.name}</option>`); 
            sS?.insertAdjacentHTML('beforeend', `<option value="${a.id}">${a.name}</option>`); 
        });
        all.forEach(a => { 
            tF?.insertAdjacentHTML('beforeend', `<option value="${a.id}">${a.emoji || ''} ${a.name}</option>`); 
            tT?.insertAdjacentHTML('beforeend', `<option value="${a.id}">${a.emoji || ''} ${a.name}</option>`); 
        });
    };
    ...
    const renderH = () => {
        ...
        let accs = financialItems.filter(i => i.type === 'account');
        if(!accs.length) { cnt.innerHTML = "<p class='placeholder-text'>No accounts added yet.</p>"; return; }
        let grid = document.createElement('div'); grid.className = 'tiles-grid';
        accs.forEach(i => {
    ...
    const renderC = () => {
        ...
        let cards = financialItems.filter(i => i.type === 'card');
        if(!cards.length) { cnt.innerHTML = "<p class='placeholder-text'>No cards added.</p>"; return; }
        let grid = document.createElement('div'); grid.className = 'tiles-grid';
        cards.forEach(i => {
            let div = document.createElement('div'); div.className = 'square-tile'; div.onclick = () => openDetails(i.id);
    ...
    const renderL = () => {
        ...
        let loans = financialItems.filter(i => i.type === 'loan');
        if(!loans.length) { cnt.innerHTML = "<p class='placeholder-text'>No loans added.</p>"; return; }
        let grid = document.createElement('div'); grid.className = 'tiles-grid';
        loans.forEach(i => {
            let div = document.createElement('div'); div.className = 'square-tile'; div.onclick = () => openDetails(i.id);
            div.innerHTML = `<div class="top-info"><div class="name">${i.name}</div><div class="sub">Due: Day ${i.dueDate}</div></div><div class="item-amount ${i.balance < 0 ? 'text-red' : ''}">£${Math.abs(i.balance).toFixed(2)}</div>`;
    ...
    const renderB = () => {
        ...
        let list = financialItems.filter(i => ['bill','salary'].includes(i.type)).map(i => {
            let dt = i.type === 'bill' ? adjPay(now.getFullYear(), now.getMonth(), i.dueDate) : adjInc(now.getFullYear(), now.getMonth(), i.dueDate);
            return { ...i, dt };
        }).sort((a,b) => a.dt - b.dt);
        if(!list.length) { cnt.innerHTML = "<p class='placeholder-text'>No items scheduled.</p>"; return; }
        let grp = document.createElement('div'); grp.className = 'settings-group';
        list.forEach(i => {
            let row = document.createElement('div'); row.className = `item-tile ${i.dt < new Date().setHours(0,0,0,0) ? 'is-paid' : ''}`;
            row.onclick = () => openEditModal(i.id);
            row.innerHTML = `<div class="item-info"><div class="list-emoji">${i.emoji}</div><div><div class="name">${i.name}</div><div class="sub">${i.dt.toLocaleDateString('en-GB',{day:'2-digit',month:'short'})}</div></div></div><div class="item-amount ${i.type === 'salary' ? 'text-green' : ''}">£${Math.abs(i.balance).toFixed(2)}</div>`;
            grp.appendChild(row);
        });
        cnt.appendChild(grp);
    };

    // Details and Modal Logic
    const openDetails = (id) => {
        ...
        let bal = document.getElementById('detail-account-balance'); 
        bal.innerText = `£${Math.abs(i.balance).toFixed(2)}`; 
        bal.className = `detail-balance-large ${i.balance < 0 ? 'text-red' : ''}`;
        
        let txL = document.getElementById('transaction-list'); txL.innerHTML = "";
        (i.transactions || []).slice(0,20).forEach(t => {
            txL.insertAdjacentHTML('beforeend', `<div class="settings-row"><div style="flex:1"><div class="tx-flex"><span>${t.description}</span><span class="${t.amount < 0 ? 'text-red' : 'text-green'}">£${Math.abs(t.amount).toFixed(2)}</span></div><div class="tx-date">${t.date}</div></div></div>`);
        });
    ...
    window.openEditModal = (id) => {
        ...
        document.getElementById('editBalance').value = Math.abs(i.balance);
        const dW = document.getElementById('edit-date-wrapper'), oW = document.getElementById('edit-od-wrapper'), cW = document.getElementById('edit-card-wrapper');
        dW.style.display = ['card','loan','bill','salary'].includes(i.type) ? 'block' : 'none';
        oW.style.display = (i.type === 'account' && i.hasOverdraft) ? 'block' : 'none';
        cW.style.display = (i.type === 'card') ? 'block' : 'none';
    ...
    document.getElementById('add-item-form').onsubmit = (e) => {
        e.preventDefault(); 
        let t = document.querySelector('input[name="itemType"]:checked').value;
        const emojiMap = { account: "🏦", card: "💳", loan: "💰", bill: "📑", salary: "💸" };
        let newItem = { id: Date.now().toString(), type: t, name: document.getElementById('itemName').value, balance: parseFloat(document.getElementById('itemAmount').value), emoji: emojiMap[t], transactions: [] };
        ...
        e.target.reset(); switchView('home');
    };

    document.getElementById('edit-form').onsubmit = (e) => {
        e.preventDefault(); 
        let idx = financialItems.findIndex(x => x.id === curId);
        if(idx > -1) {
            ...
            if(financialItems[idx].type === 'account') financialItems[idx].odLimit = parseFloat(document.getElementById('editOdLimit').value) || 0;
            if(financialItems[idx].type === 'card') financialItems[idx].creditLimit = parseFloat(document.getElementById('editCardLimit').value) || 0;
            localStorage.setItem('financialItems', JSON.stringify(financialItems)); closeAllModals(); renderAll();
        }
    };

    document.getElementById('delete-btn').onclick = () => { if(confirm("Delete item permanently?")) { financialItems = financialItems.filter(x => x.id !== curId); localStorage.setItem('financialItems', JSON.stringify(financialItems)); closeAllModals(); switchView('home'); } };

    document.getElementById('transaction-form').onsubmit = (e) => {
        e.preventDefault(); 
        let fId = document.getElementById('txAccount').value, type = document.querySelector('input[name="txType"]:checked').value, amt = parseFloat(document.getElementById('txAmount').value), desc = document.getElementById('txDesc').value;
